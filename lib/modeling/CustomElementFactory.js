...
export default class CustomElementFactory {
  ...
  _ensureId(element) {
    if (!element.id && this._needsId(element)) {
      ...
      throw new Error(
        'Cannot create element, id "' + element.id + '" already exists',
      );
    }
  }
  ...
  createDiShape(businessObject, bounds, attrs) {
    return this.create(
      `${MODELER_DI_PREFIX}:DiagramShape`,
      assign(
        {
  ...
  // CustomModelerTodo: Modify this function to return the default size for each element type.
