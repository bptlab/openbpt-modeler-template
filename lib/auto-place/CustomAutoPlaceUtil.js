...
export function getNewShapePosition(source, element) {
  if (
    element.type === `${MODELER_PREFIX}:Place` ||
    element.type === `${MODELER_PREFIX}:Transition`
  ) {
...
export function getFlowNodePosition(source, element) {
  ...
  // CustomModelerTodo: Define the default horizontal distance between elements.
  ...
  // CustomModelerTodo: Define the margin and minimum vertical distance between elements.
  const margin = 30;
